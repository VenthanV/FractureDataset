...
# For fracture screening, sensitivity > specificity in clinical priority.
# Threshold 0.40 gives ~0.825 sensitivity vs 0.679 at Youden's J (0.613).
# Override per deployment via env var; always validate against eval_results.json.
SENSITIVITY_TARGET_THRESHOLD = float(os.getenv("SENSITIVITY_TARGET_THRESHOLD", "0.40"))
...
# ── Class Weights ──────────────────────────────────────────────────────────
# Dataset is ~balanced (48.5/51.5%), so inverse-frequency weights ≈ 1.0 each
# and provide no sensitivity push. Use intentional asymmetric weighting instead:
#   CLASS_WEIGHT_FRACTURE=1.0  → no effect (default, balanced)
#   CLASS_WEIGHT_FRACTURE=2.0  → recommended starting point
#   CLASS_WEIGHT_FRACTURE=3.0  → aggressive sensitivity push
# Expected: each unit increase → +3–5% sensitivity, −3–5% specificity tradeoff.
# Weight tensor passed to CrossEntropyLoss: [normal=1.0, fracture=CLASS_WEIGHT_FRACTURE]
CLASS_WEIGHT_FRACTURE = float(os.getenv("CLASS_WEIGHT_FRACTURE", "1.0"))

# ── MixUp Augmentation ─────────────────────────────────────────────────────
# Mixes random image pairs to reduce overconfidence and improve calibration.
# Enable with USE_MIXUP=1. If active, reduce LABEL_SMOOTHING to 0.05 to avoid
# over-regularisation (both act on the same logits).
USE_MIXUP   = bool(int(os.getenv("USE_MIXUP",   "0")))
MIXUP_ALPHA = float(os.getenv("MIXUP_ALPHA", "0.2"))

# ── Outlier Exclusion ──────────────────────────────────────────────────────
# Optional file listing image paths to exclude from training (one path per line).
# Generated by notebooks/data_quality.ipynb for quality-flagged images.
# Only applied to train split — val/test remain intact for honest evaluation.
_exclude_candidate = Path(os.getenv("EXCLUDE_PATHS_FILE", str(Path(__file__).parent.parent / "data" / "exclude_paths.txt")))
EXCLUDE_PATHS_FILE: Path | None = _exclude_candidate if _exclude_candidate.exists() else None
